...
import numpy as np
import torch
from  geometry import axis_angle_to_quaternion, quaternion_to_axis_angle, matrix_to_quaternion, quaternion_to_matrix
...
def slerp_poses(last_pose, new_pose, number_of_frames, pose_rep="matrix"):
    # interpolation
    from easyconvert import to_matrix, matrix_to
...
def align_interpolate(rotations, translation, s, e, align_rot=True, align_trans=True, interpolate=True):
    ...
    from interpolation import align_orientations, interpolate_track, linear_interp
...
def smooth_poses(last_pose,newpose,alpha=0.8, pose_rep="matrix"):
    from easyconvert import to_matrix, matrix_to
    last_pose_matrix = to_matrix( pose_rep, last_pose)
    new_pose_matrix = to_matrix( pose_rep, newpose)

    last_pose_quat = matrix_to("quaternion", last_pose_matrix).numpy()
    new_pose_quat = matrix_to("quaternion", new_pose_matrix).numpy()

    inter = (1-alpha) * last_pose_quat + alpha * new_pose_quat
    inter_matrix = to_matrix("quaternion", torch.from_numpy(inter))

    inter_poserep = matrix_to( pose_rep, inter_matrix)
    return inter_poserep
